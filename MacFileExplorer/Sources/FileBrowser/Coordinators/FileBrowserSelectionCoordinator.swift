...
final class FileBrowserSelectionCoordinator {
    ...
    func currentSingleSelection() -> FileItem? {
        ...
        // Use the common `selectedItems` logic but optimization for single item check could be added if needed.
        // For now, consistent behavior is most important.
        let items = selectedItems(includeClickedRowFallback: true)
        return items.count == 1 ? items.first : nil
    }
    ...
    func selectAll() {
        ...
        case .list:
            delegate.outlineView.selectAll(nil)
        case .icons, .windowsList:
            delegate.collectionView?.selectAll(nil)
        case .columns:
            delegate.browserView?.selectAll(nil)
        }
        
        // Trigger status update manually as selectAll might not always trigger delegate callbacks immediately
        updateStatusBar()
    }
