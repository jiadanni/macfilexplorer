...
protocol SettingsStoreDelegate: AnyObject {
    ...
    /// Called when the terminal visibility changes.
    func settingsStore(_ settingsStore: SettingsStoreProtocol, terminalVisibilityDidChange isVisible: Bool)
}

/// Default implementations for SettingsStoreDelegate methods to make them optional
extension SettingsStoreDelegate {
    func settingsStore(_ settingsStore: SettingsStoreProtocol, previewPaneVisibilityDidChange isVisible: Bool) {}
    func settingsStore(_ settingsStore: SettingsStoreProtocol, hiddenFilesStateDidChange isVisible: Bool) {}
    func settingsStoreDidUpdateGlobalFolderColor(_ settingsStore: SettingsStoreProtocol) {}
    func settingsStore(_ settingsStore: SettingsStoreProtocol, showFileExtensionsDidChange show: Bool) {}
...
final class SettingsStore: SettingsStoreProtocol {
    ...
    // MARK: - Grouped Settings Access
    /// Access to organized setting groups for better code clarity.
    /// See SettingsStoreGroups.swift for available groups.
    
    var appearance: AppearanceSettings {
        AppearanceSettings(store: self)
    }
    
    var view: ViewSettings {
        ViewSettings(store: self)
    }
    
    var fileOperations: FileOperationsSettings {
        FileOperationsSettings(store: self)
    }
    
    var uiVisibility: UIVisibilitySettings {
        UIVisibilitySettings(store: self)
    }
    
    var contextMenu: ContextMenuSettings {
        ContextMenuSettings(store: self)
    }
    
    var sidebar: SidebarSettings {
        SidebarSettings(store: self)
    }
    
    var goMenu: GoMenuSettings {
        GoMenuSettings(store: self)
    }
    
    var startPage: StartPageSettings {
        StartPageSettings(store: self)
    }
    
    var windowLayout: WindowLayoutSettings {
        WindowLayoutSettings(store: self)
    }
    
    var terminal: TerminalSettings {
        TerminalSettings(store: self)
    }
    
    var other: OtherSettings {
        OtherSettings(store: self)
    }
    ...
    /// Current zoom level for the icon view.
    /// Default: `1.0`
    var zoomLevel: Double {
        get { defaults.object(forKey: UserDefaults.Keys.zoomLevel.rawValue) as? Double ?? 1.0 }
        set { 
            defaults.set(newValue, forKey: UserDefaults.Keys.zoomLevel.rawValue)
            notifyDelegates { _ in } // No specific delegate method yet
        }
    }
    ...
    var terminalIsVisible: Bool {
        ...
        set { 
            defaults.set(newValue, forKey: AdditionalKeys.terminalIsVisible)
            notifyDelegates { delegate in
                delegate.settingsStore(self, terminalVisibilityDidChange: newValue)
            }
        }
    }
